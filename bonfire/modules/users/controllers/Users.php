...
class Users extends Front_Controller
{
    ...
    public function login()
    {
        ...
        if (isset($_POST['log-me-in']))
        {
			...
			if(strtoupper($this->input->post('captcha_st')) != strtoupper($this->session->userdata('captcha_str'))){
			   Template::set_message("Captcha Salah", 'error');
				//die($this->input->post('captcha_st') ." ".$this->session->userdata('captcha_str'));   
		   }else{
        ...
        $vals = array(
            ...
            'img_width' => '190', //lebar gambar captcha
            'img_height' => '40', //tinggi gambar captcha
            'expiration' => 7200 //expired time per captcha
        );
 		
        $cap = create_captcha($vals);
        $captcha_image = $cap['image']; //variable
        
        Template::set('captcha_image', $captcha_image);
        Template::set('page_title', 'Login');
        Template::render('login');
    }
	public function validate_captcha(){
		if($this->input->post('captcha_st') != $this->session->userdata('captcha_str')){
